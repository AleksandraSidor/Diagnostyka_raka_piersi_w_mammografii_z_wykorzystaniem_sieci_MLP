...
```{r dirty_data_analysis, echo=TRUE}
#-------------------------histogramy pierwsze---------------------

bi_rads_h <- hist(new_data$bi_rads, main="Ocena BI-RADS", xlab="Wartosc", ylab="Ilosc wystapien", xlim=c(0,max(new_data$bi_rads, na.rm = TRUE)), ylim=c(0, 600), breaks=56, col="#9ACD32")
...
shape_h <- hist(new_data$shape, main="Ksztalt guza", xlab="Ksztalt", ylab="Ilosc wystapien", xlim=c(0,4), ylim=c(0, 450), breaks=0:4, col="#B0E0E6", xaxt="n")
...
margin_h <- hist(new_data$margin, main="Margines guza", xlab="Margines", ylab="Ilosc wystapien", xlim=c(0,max(new_data$margin, na.rm = TRUE)), ylim=c(0, 400), breaks=0:5, col="#DEB887", xaxt="n")
...
density_h <- hist(new_data$density, main="Gestosc guza", xlab="Gestosc guza", ylab="Ilosc wystapien", xlim=c(0,max(new_data$density, na.rm = TRUE)), ylim=c(0, 850), breaks=0:4, col="#BA55D3", xaxt="n")
...
Wartosc bi_rads=55 powstala prawdopodobnie w wyniku bledu podczas tworzenia
zbioru danych Mammographic Mass. Zakladamy, ze chodzilo o wartosc 5 i na taka
podmieniamy ewidentnie bledna wartosc:
...
```{r clean_data_analysis}
#-------------------------histogramy po czyszczeniu---------------------

bi_rads_new_h <- hist(data_clean$bi_rads, main="Ocena BI-RADS", xlab="Wartosc", ylab="Ilosc wystapien", xlim=c(0,max(data_clean$bi_rads, na.rm = TRUE)), ylim=c(0, 500), breaks=5, col="#9ACD32", xaxt="n")
...
Do zakresu [-1.0; 1.0], bez labeli (czyli bez cechy severity):
...
Na calym zbiorze, bez podzialu na zbior treningowy i testowy:
...
## 10 pierwszych predykcji vs. targetow
knitr::kable(head(predsVStarget, 10), caption = "10 pierwszych predykcji sieci")
```
